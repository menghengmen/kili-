...
+ (NSString*)get_HH_MM_fromMinutes:(NSInteger)minutes{
    NSInteger hour, minute;
    minute = minutes % 60;
    hour = minutes / 60;
    NSString* hh_mm = [NSString stringWithFormat:@"%02zd:%02zd", hour, minute];
    return hh_mm;
}

+ (NSInteger)getCurDayMinites{
    return [[self getCurHourString] integerValue] * 60 + [[self getCurMinuteString] integerValue];
}
@end
