...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay showMsg:(NSString *)msg forVC:(UIViewController *)vc WithActionBlock:(void (^)(void))block{
}

- (void)hideDelayed:(NSNumber *)animated showMsg:(NSString *)msg forVC:(UIViewController *)vc WithActionBlock:(void (^)(void))block{
    [self hide:[animated boolValue]];
}
...
- (void)done {
	...
	isFinished = YES;
	self.alpha = 0.0f;
    if(self.mode == MBProgressHUDModeCustomView&&!self.customView.hidden&&[self.customView isKindOfClass:[UIImageView class]]&&self.customView.tag==1994){
